...
class KeyboardDetector:
    ...
    def _verify_middle_c(self, keyboard: PianoKeyboard) -> bool:
        ...
        if abs(middle_c.brightness - keyboard.white_baseline) > self._perceptual_threshold:
            print(f"Middle C verification failed: Middle C brightness ({middle_c.brightness}) "
                  f"is outside of the white key baseline ({keyboard.white_baseline} ± {self._perceptual_threshold}). "
        ...
        for i, expected_color in enumerate(expected_colors):
            ...
            if expected_color == 'W':
                if abs(key.brightness - keyboard.white_baseline) > self._perceptual_threshold:
                    print(f"Middle C verification failed: Expected white key at index {key_index} "
                          ...
                          f"white baseline ({keyboard.white_baseline} ± {self._perceptual_threshold})")
                    return False
            else:  # expected_color == 'B'
                if abs(key.brightness - keyboard.black_baseline) > self._perceptual_threshold:
                    print(f"Middle C verification failed: Expected black key at index {key_index} "
                          f"({key.name}), but brightness ({key.brightness}) is outside of the "
                          f"black baseline ({keyboard.black_baseline} ± {self._perceptual_threshold})")
                    return False
