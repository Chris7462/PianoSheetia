...
class TestPianoKeyboard(unittest.TestCase):
    ...
    def test_update_key_position(self):
        ...
        # Test valid update
        self.keyboard.update_key_position(0, 100, 200)
        print(self.keyboard)
        key = self.keyboard[0]
        self.assertEqual(key.x, 100)
        self.assertEqual(key.y, 200)
        ...
        with self.assertRaises(IndexError):
            self.keyboard.update_key_position(-1, 100, 200)

        with self.assertRaises(IndexError):
            self.keyboard.update_key_position(88, 100, 200)

    def test_update_key_brightness(self):
        """Test updating key brightness"""
        # Test valid update
        self.keyboard.update_key_brightness(0, 0.75)
        print(self.keyboard)
        key = self.keyboard[0]
        ...
        # Test invalid index
        with self.assertRaises(IndexError):
            self.keyboard.update_key_brightness(-1, 0.5)

        with self.assertRaises(IndexError):
            self.keyboard.update_key_brightness(88, 0.5)
    ...
    def test_repr_method(self):
        ...
        self.keyboard.update_key_position(0, 100, 200)
        self.keyboard.update_key_brightness(0, 0.5)
